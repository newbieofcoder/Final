...
const EditXeScreen = ({ route, navigation }) => {
  const { xe } = route.params;
  const dispatch = useDispatch();

  useEffect(() => {
    // Debug log
    console.log("Received xe data:", xe);
    console.log("Xe ID:", xe.id);
  }, [xe]);

  const [formData, setFormData] = useState({
    id: xe.id,
    ten_xe_ph56584: xe.ten_xe_ph56584,
    mau_sac_ph56584: xe.mau_sac_ph56584,
    gia_ban_ph56584: xe.gia_ban_ph56584.toString(),
    mo_ta_ph56584: xe.mo_ta_ph56584,
    hinh_anh_ph56584: xe.hinh_anh_ph56584,
  });
  ...
  const showSuccessToast = () => {
    Toast.show({
      ...
      text2: "Thông tin xe đã được cập nhật",
      position: "top",
      visibilityTime: 2000,
    });
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      Alert.alert(
        "Lỗi cập nhật",
        `Không thể cập nhật thông tin xe. Chi tiết lỗi: ${error.message}`
      );
    } finally {
  ...
  return (
    <ScrollView style={styles.container}>
      <View style={styles.form}>
        ...
        <CustomButton
          title={isSubmitting ? "Đang cập nhật..." : "Cập nhật"}
